...
| `-DMULTIFLOATS_BUILD_SHARED=ON`     | OFF     | Also build a versioned shared library (`libmultifloats.so.N` / `libmultifloats.N.dylib`) from the same objects as the static archive — installed with the usual soname + dev symlinks, `SOVERSION` tracking `MULTIFLOATS_ABI_VERSION`, ELF-hardened (full RELRO + `BIND_NOW`, `--no-undefined`). With hidden visibility (default) its dynamic symbol table exports only the `extern "C"` C ABI — for direct `-lmultifloats` / `dlopen` consumers; `find_package` still routes to the static archive. The release ships one prebuilt shared object per platform. |
