...
error check (independent across ops). Inputs are genuine double-doubles:
each leading double is perturbed by a random ~1 ULP offset so the lo
limb is non-zero, matching the Fortran fuzz methodology.
...
|---|---|---|---|---|---|---|
| add | Julia: two\_sum EFT | **2.4×** | †| **4.1×** | **4.5×** | 1.3e-32 |
| sub | Julia: two\_sum EFT (negate + add) | **2.7×** | †| **4.2×** | **4.6×** | 1.2e-32 |
| mul | Julia: two\_prod EFT via FMA | **10×** | †| **7.7×** | **9.4×** | 2.8e-32 |
| div | original: Newton refinement (1/y seed, one step) | **2.1×** | †| **2.6×** | 1.2× | 4.4e-32 |
| sqrt | Julia: Karp–Markstein (reciprocal sqrt seed + Newton) | **50×** | 1.7e-32 | **29×** | **55×** | 2.5e-32 |
| cbrt | original: Newton correction on cbrt(hi) seed | **41×** | †| **13×** | **17×** | 1.7e-31 |
| fma | original: x\*y + z via DD ops | **61×** | †| **69×** | **75×** | 2.5e-30 |
| abs | original: sign-check + negate limbs | 1.9× | exact | **5.1×** | **7.8×** | exact |
| neg | original: negate both limbs | **2.1×** | exact | **3.6×** | **5.8×** | exact |

### Rounding

| op | approach | M1 Max × | M1 Max err | Skylake × | Raptor Lake × | Raptor Lake err |
|---|---|---|---|---|---|---|
| floor | original: floor hi, adjust lo | **3.1×** | exact | **4.2×** | **5.7×** | exact |
| ceil | original: ceil hi, adjust lo | **3.1×** | exact | **3.9×** | **5.6×** | exact |
| trunc | original: signbit ? −floor(−x) : floor(x) | **2.5×** | exact | **3.2×** | **4.9×** | exact |
| round | original: trunc(x + ½·sign(x)) | 0.98× | exact | 1.0× | 1.3× | exact |
| rint | original: nearbyint on hi, adjust lo | **11×** | exact | **11×** | **19×** | exact |
| nearbyint | original: nearbyint on hi, adjust lo | **34×** | exact | **65×** | **149×** | exact |

### Binary

| op | approach | M1 Max × | M1 Max err | Skylake × | Raptor Lake × | Raptor Lake err |
|---|---|---|---|---|---|---|
| fmin | original: DD comparison + select | **6.1×** | exact | **9.8×** | **7.8×** | exact |
| fmax | original: DD comparison + select | **5.9×** | exact | **13×** | **11×** | exact |
| fdim | original: DD comparison, then subtract or zero | **5.8×** | †| **6.9×** | **7.3×** | 1.2e-32 |
| copysign | original: sign-bit copy to hi, propagate to lo | 1.9× | exact | **5.2×** | **12×** | exact |
| fmod | sample: floor-multiple reduction loop; fallback to div chain | 0.86× | †| 0.76× | 1.1× | 2.1e-30 |
| hypot | original: scaled sqrt(x²+y²) | **37×** | †| **20×** | **38×** | 2.7e-32 |
| ldexp(.,5) | original: ldexp on both limbs | **2.1×** | exact | **2.4×** | **2.9×** | exact |

### Exponential / logarithmic

| op | approach | M1 Max × | M1 Max err | Skylake × | Raptor Lake × | Raptor Lake err |
|---|---|---|---|---|---|---|
| exp | Julia: exp2 polynomial (14-term Horner) + ldexp reconstruction | **3.0×** | 8.9e-19 | **2.7×** | **3.9×** | 8.8e-19 |
| exp2 | Julia: exp2 polynomial (14-term Horner) | **3.2×** | 8.9e-19 | **3.1×** | **4.4×** | 8.9e-19 |
| expm1 | original: exp(x) − 1 via DD sub | **4.2×** | 3.0e-18 | **3.3×** | **4.0×** | 2.9e-18 |
| log | Julia: log2 table lookup (32 centers) + polynomial (7-term Horner) | **4.4×** | 9.0e-17 | **4.0×** | **6.2×** | 5.0e-16 |
| log10 | Julia: log2 kernel × DD log10(2) | **5.8×** | 9.0e-17 | **5.1×** | **6.8×** | 5.0e-16 |
| log2 | Julia: log2 table lookup + polynomial | **5.7×** | 9.0e-17 | **4.7×** | **6.3×** | 5.0e-16 |
| log1p | original: log(1 + x) via DD add | **4.7×** | 1.2e-19 | **4.3×** | **5.2×** | 1.4e-19 |
| pow | Julia: exp(y × log(x)) | **4.2×** | 1.5e-18 | **4.1×** | **5.2×** | 9.7e-18 |

### Trigonometric

| op | approach | M1 Max × | M1 Max err | Skylake × | Raptor Lake × | Raptor Lake err |
|---|---|---|---|---|---|---|
| sin | original: 13-term Taylor Horner + 3-part Cody–Waite π/2 + π/8 split | **2.8×** | †| **2.3×** | **2.9×** | 2.7e-31 |
| cos | original: 13-term Taylor Horner + 3-part Cody–Waite π/2 + π/8 split | **2.9×** | †| **2.3×** | **3.4×** | 1.6e-30 |
| tan | original: sin/cos Taylor kernels + DD divide | 1.2× | 5.4e-32 | 1.1× | 1.4× | 5.4e-32 |
| asin | original: Newton step on sin, seeded by libm asin(hi) | 1.8× | †| 1.7× | **2.5×** | 4.9e-32 |
| acos | original: Newton step on cos, seeded by libm acos(hi) | 1.9× | †| 1.7× | **2.3×** | 3.4e-32 |
| atan | original: Newton on tan + atan(x)=π/2·sign(x)−atan(1/x) for \|x\|>1 | 0.93× | †| 1.0× | 1.1× | 3.0e-32 |
| atan2 | original: Newton step on atan + quadrant correction | 1.0× | †| 1.1× | 1.2× | 5.3e-32 |

### Hyperbolic

| op | approach | M1 Max × | M1 Max err | Skylake × | Raptor Lake × | Raptor Lake err |
|---|---|---|---|---|---|---|
| sinh | original: Taylor series (\|x\|<0.1) or (exp−exp⁻¹)/2 | **2.3×** | 9.3e-19 | 1.9× | **2.2×** | 9.0e-19 |
| cosh | original: (exp+exp⁻¹)/2 | 1.7× | 8.9e-19 | 1.5× | **2.1×** | 8.8e-19 |
| tanh | original: sinh/cosh (\|x\|<0.5) or (1−e⁻²ˣ)/(1+e⁻²ˣ) | **2.3×** | 6.6e-19 | **2.2×** | **2.6×** | 7.1e-19 |
| asinh | original: Taylor series (\|x\|<0.01) or log(x+√(x²+1)) with Newton | **6.0×** | 7.7e-20 | **6.0×** | **8.2×** | 8.0e-20 |
| acosh | original: log(x+√(x²−1)) with Newton correction | **5.4×** | 2.8e-20 | **5.5×** | **7.1×** | 3.6e-20 |
| atanh | original: Taylor series (\|x\|<0.01) or ½·log((1+x)/(1−x)) | **4.4×** | 5.1e-16 | **4.4×** | **5.5×** | 5.2e-16 |

### Error / special functions

| op | approach | M1 Max × | M1 Max err | Skylake × | Raptor Lake × | Raptor Lake err |
|---|---|---|---|---|---|---|
| erf | original: Taylor series (\|x\|<2) + libm erf(hi) deriv correction | **4.7×** | 2.4e-18 | **5.7×** | **7.8×** | 3.7e-18 |
| erfc | original: 1−erf(x), or libm erfc(hi) for large x | **4.6×** | 5.7e-16 | **5.5×** | **7.9×** | 3.8e-15 |
| tgamma | original: libm tgamma(hi), no lo correction | **120×** | 1.2e-16 | **59×** | **88×** | 2.2e-15 |
| lgamma | original: libm lgamma(hi), no lo correction | **69×** | 4.4e-16 | **46×** | **57×** | 8.0e-14 |
