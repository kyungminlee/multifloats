...
|---|---|---|---|---|---|---|---|---|
| add | Julia: two\_sum EFT | full DD | 1.2× | 1.5e-32 | **3.6×** | 1.5e-32 | **4.1×** | 1.5e-32 |
| sub | Julia: two\_sum EFT (negate + add) | full DD | **2.0×** | 5.7e-33 | **3.1×** | 6.2e-33 | **3.6×** | 6.2e-33 |
| mul | Julia: two\_prod EFT via FMA | full DD | **11×** | 2.1e-32 | **5.9×** | 3.1e-32 | **5.9×** | 3.3e-32 |
| div | original: Newton refinement (1/y seed, one step) | full DD | **3.6×** | 3.8e-32 | **3.2×** | 6.1e-32 | 0.93× | 5.4e-32 |
| sqrt | Julia: Karp–Markstein (reciprocal sqrt seed + Newton) | full DD | **36×** | 4.5e-32 | **17×** | 5.4e-32 | **33×** | 5.2e-32 |
| add (mf+dp) | Julia: two\_sum EFT | exact | **2.0×** | exact | **4.3×** | exact | **4.4×** | exact |
| mul (dp\*mf) | Julia: two\_prod EFT via FMA | full DD | **8.8×** | 2.1e-32 | **6.1×** | 3.1e-32 | **5.3×** | 3.3e-32 |

### Unary

| op | approach | prec | M1 Max × | M1 Max err | Skylake × | Skylake err | Raptor Lake × | Raptor Lake err |
|---|---|---|---|---|---|---|---|---|
| abs | original: sign-check + negate limbs | exact | 1.1× | exact | **2.4×** | exact | **3.4×** | exact |
| neg | original: negate both limbs | exact | **2.4×** | exact | **4.4×** | exact | **5.2×** | exact |
| aint | original: truncate hi, check DD fractional part | exact | 1.8× | exact | 1.4× | exact | 1.4× | exact |
...
| set\_exponent | original: scale + set\_exponent on hi | exact | 1.7× | exact | **3.3×** | exact | **5.9×** | exact |

### Binary

| op | approach | prec | M1 Max × | M1 Max err | Skylake × | Skylake err | Raptor Lake × | Raptor Lake err |
|---|---|---|---|---|---|---|---|---|
| min | original: DD comparison + select | full DD | **2.2×** | 5.5e-33 | **4.2×** | 5.9e-33 | **5.1×** | 6.2e-33 |
| max | original: DD comparison + select | full DD | **2.3×** | 5.3e-33 | **4.3×** | 6.0e-33 | **5.1×** | 6.1e-33 |
| min3 | original: chained min | full DD | 1.8× | 5.7e-33 | **6.9×** | 5.8e-33 | **3.6×** | 5.5e-33 |
| max3 | original: chained max | full DD | **2.4×** | 6.1e-33 | **6.4×** | 5.7e-33 | **5.8×** | 5.8e-33 |
| sign | original: sign-check + negate | exact | 1.2× | exact | **2.3×** | exact | **2.4×** | exact |
| dim | original: DD comparison, then subtract or zero | full DD | 1.6× | 5.5e-33 | **4.3×** | 5.9e-33 | **3.6×** | 6.2e-33 |
| hypot | original: scaled sqrt(x²+y²) | full DD | **6.1×** | 6.4e-32 | **5.4×** | 7.2e-32 | **6.5×** | 7.9e-32 |
| mod | sample: floor-multiple reduction loop; fallback to div chain | full DD | 0.59× | 5.0e-33 | 1.1× | 3.2e-32 | 0.87× | 2.0e-32 |
| modulo | original: mod + sign adjustment | full DD | 1.2× | 5.0e-33 | 1.6× | 3.2e-32 | 1.4× | 2.0e-32 |
...
| log | Julia: log2 table lookup (32 centers) + polynomial (7-term Horner) | full DD | **4.5×** | 1.6e-32 | **3.8×** | 3.0e-32 | **5.6×** | 4.4e-32 |
| log10 | Julia: log2 kernel × DD log10(2) | full DD | **5.8×** | 1.5e-32 | **4.9×** | 3.4e-32 | **7.2×** | 2.9e-32 |
| pow | Julia: exp(y × log(x)) | full DD | **4.2×** | 5.2e-32 | **4.1×** | 2.2e-30 | **5.3×** | 2.2e-30 |
| pow\_int | original: repeated squaring via DD mul | full DD | **18×** | 1.7e-32 | **5.6×** | 2.2e-32 | **7.3×** | 2.4e-32 |
...
| cospi | Julia: cospi Horner polynomial, direct | full DD | **3.5×** | 8.2e-27 | **2.6×** | 8.2e-27 | **3.8×** | 8.2e-27 |
| tan | original: sin/cos Taylor kernels + DD divide | full DD | 0.89× | 4.7e-32 | 0.8× | 6.7e-32 | 1.1× | 6.1e-32 |
| asin | original: Newton step on sin, seeded by libm asin(hi) | full DD | 1.4× | 6.4e-33 | 1.2× | 4.3e-32 | 1.6× | 4.7e-32 |
| acos | original: Newton step on cos, seeded by libm acos(hi) | full DD | 1.5× | 3.6e-33 | 1.2× | 5.4e-32 | 1.7× | 2.9e-32 |
| atan | original: Newton on tan + atan(x)=π/2·sign(x)−atan(1/x) for \|x\|>1 | full DD | 0.77× | 3.6e-32 | 0.7× | 4.2e-32 | 0.92× | 5.1e-32 |
...
| op | approach | prec | M1 Max × | M1 Max err | Skylake × | Skylake err | Raptor Lake × | Raptor Lake err |
|---|---|---|---|---|---|---|---|---|
| sinh | original: Taylor series (\|x\|<0.1) or (exp−exp⁻¹)/2 | full DD | **2.3×** | 4.4e-31 | 1.9× | 3.7e-30 | **2.7×** | 4.2e-30 |
...
| acosh | original: log(x+√(x²−1)) with Newton correction | full DD | **5.4×** | 7.4e-33 | **4.9×** | 4.1e-32 | **6.1×** | 3.5e-32 |
| atanh | original: Taylor series (\|x\|<0.01) or ½·log((1+x)/(1−x)) | full DD | **4.5×** | 4.0e-31 | **4.3×** | 1.2e-30 | **6.9×** | 1.3e-30 |

### Error / special functions

| op | approach | prec | M1 Max × | M1 Max err | Skylake × | Skylake err | Raptor Lake × | Raptor Lake err |
|---|---|---|---|---|---|---|---|---|
| erf | original: Taylor series (\|x\|<2) + libm erf(hi) deriv correction | deriv-corrected | **5.1×** | 3.2e-20 | **5.4×** | 7.8e-19 | **8.1×** | 3.7e-19 |
| erfc | original: 1−erf(x), or libm erfc(hi) for large x | deriv-corrected | **5.0×** | 2.4e-16 | **5.5×** | 2.5e-16 | **8.0×** | 2.1e-16 |
| erfc\_scaled | original: libm erfc\_scaled(hi), no lo correction | single-double | **185×** | 4.3e-16 | **148×** | 5.7e-16 | **198×** | 4.9e-16 |
| gamma | original: libm gamma(hi), no lo correction | single-double | **111×** | 8.4e-17 | **42×** | 4.1e-16 | **58×** | 3.4e-16 |
...
| bessel\_j1 | original: libm bessel\_j1(hi), no lo correction | single-double | **102×** | 2.7e-16 | **69×** | 1.7e-13 | **91×** | 3.0e-15 |
| bessel\_jn(3,.) | original: libm bessel\_jn(3,hi), no lo correction | single-double | **104×** | 3.1e-16 | **67×** | 7.4e-15 | **83×** | 3.0e-15 |
| bessel\_y0 | original: libm bessel\_y0(hi), no lo correction | single-double | **136×** | 1.9e-16 | **75×** | 4.9e-16 | **94×** | 9.6e-16 |
| bessel\_y1 | original: libm bessel\_y1(hi), no lo correction | single-double | **157×** | 2.4e-16 | **71×** | 1.7e-15 | **95×** | 1.5e-15 |
| bessel\_yn(3,.) | original: libm bessel\_yn(3,hi), no lo correction | single-double | **144×** | 1.9e-15 | **75×** | 1.3e-14 | **108×** | 2.9e-14 |

### Complex arithmetic

| op | approach | prec | M1 Max × | M1 Max err | Skylake × | Skylake err | Raptor Lake × | Raptor Lake err |
|---|---|---|---|---|---|---|---|---|
| cx\_add | original: component-wise DD add | full DD | **2.4×** | 1.3e-32 | **3.6×** | 1.3e-32 | **3.0×** | 1.2e-32 |
...
| cx\_conjg | original: negate im limbs | exact | 1.9× | exact | **3.5×** | exact | **4.2×** | exact |
| cx\_abs | original: hypot(re, im) | full DD | **3.9×** | 6.7e-32 | **4.9×** | 6.6e-32 | **5.9×** | 7.9e-32 |

### Complex transcendentals

| op | approach | prec | M1 Max × | M1 Max err | Skylake × | Skylake err | Raptor Lake × | Raptor Lake err |
|---|---|---|---|---|---|---|---|---|
| cx\_sqrt | original: Kahan-style (\|z\|+\|a\|)/2 with scaling | full DD | **5.7×** | 4.9e-32 | **4.6×** | 6.4e-32 | **6.1×** | 6.5e-32 |
...
| cx\_tan | original: complex sin/cos ratio | full DD | 0.80× | 8.0e-31 | 0.7× | 3.1e-30 | 0.98× | 1.6e-30 |
| cx\_sinh | original: sinh(re)cos(im), cosh(re)sin(im) | full DD | 1.6× | 5.9e-32 | 1.5× | 1.2e-30 | **2.0×** | 5.4e-30 |
| cx\_cosh | original: cosh(re)cos(im), sinh(re)sin(im) | full DD | 1.6× | 7.3e-32 | 1.5× | 1.2e-30 | **2.1×** | 5.4e-30 |
| cx\_tanh | original: complex tanh via sinh/cosh | full DD | 0.84× | 8.2e-31 | 0.8× | 1.1e-30 | 1.1× | 8.6e-31 |
| cx\_asin | original: −i·log(iz+√(1−z²)) | deriv / full DD | **2.2×** | 2.3e-27 (re) / 1.4e-31 (im) | **2.4×** | 2.8e-23 (re) / 8.4e-31 (im) | **3.3×** | 2.5e-23 (re) / 1.0e-30 (im) |
...
| cx\_asinh | original: log(z+√(z²+1)) | deriv / full DD | **2.2×** | 2.7e-26 (re) / 2.0e-32 (im) | **2.2×** | 6.7e-22 (re) / 6.6e-32 (im) | **2.8×** | 6.9e-23 (re) / 7.1e-32 (im) |
| cx\_acosh | original: log(z+√(z²−1)) | full DD | **2.1×** | 4.4e-32 (re) / 5.8e-33 (im) | **2.0×** | 7.2e-31 (re) / 2.9e-32 (im) | **2.6×** | 7.1e-31 (re) / 2.2e-32 (im) |
...
| arr\_product (n=8) | original: chained DD mul | full DD | **3.2×** | 1.1e-53 | **2.2×** | 4.3e-49 | **3.5×** | 4.0e-50 |
| arr\_maxval (n=8) | original: chained DD compare | full DD | **3.1×** | 2.9e-33 | **5.8×** | 6.1e-33 | **7.7×** | 5.9e-33 |
| arr\_minval (n=8) | original: chained DD compare | full DD | **3.4×** | 2.7e-33 | **5.2×** | 6.0e-33 | **4.2×** | 6.0e-33 |
| arr\_dot (n=8) | original: fused multiply-accumulate with periodic renormalization | full DD | **4.3×** | 2.5e-32 | **4.6×** | 1.0e-31 | **7.3×** | 2.1e-31 |
| arr\_norm2 (n=8) | original: sqrt(dot(x,x)) | full DD | **4.3×** | 1.4e-32 | **6.8×** | 6.4e-32 | **5.9×** | 5.2e-32 |
| arr\_matmul (8×8\*8) | original: fused multiply-accumulate with periodic renormalization | full DD | **2.1×** | 5.8e-32 | 0.8× | 8.8e-30 | 0.94× | 7.1e-30 |
...
|---|---|---|---|---|
| add | Julia: two\_sum EFT | **2.6×** | **4.1×** | **5.0×** |
| sub | Julia: two\_sum EFT (negate + add) | **2.8×** | **4.2×** | **4.3×** |
| mul | Julia: two\_prod EFT via FMA | **12×** | **7.7×** | **5.6×** |
...
| fma | original: x\*y + z via DD ops | **63×** | **69×** | **126×** |
| abs | original: sign-check + negate limbs | **2.0×** | **5.1×** | **5.8×** |
| neg | original: negate both limbs | **2.1×** | **3.6×** | **4.4×** |

### Rounding

| op | approach | M1 Max × | Skylake × | Raptor Lake × |
|---|---|---|---|---|
| floor | original: floor hi, adjust lo | **3.2×** | **4.2×** | **5.5×** |
| ceil | original: ceil hi, adjust lo | **3.2×** | **3.9×** | **5.6×** |
| trunc | original: signbit ? −floor(−x) : floor(x) | **2.5×** | **3.2×** | **4.8×** |
| round | original: trunc(x + ½·sign(x)) | 0.98× | 1.0× | 1.3× |
| rint | original: nearbyint on hi, adjust lo | **11×** | **11×** | **19×** |
| nearbyint | original: nearbyint on hi, adjust lo | **36×** | **65×** | **143×** |

### Binary

| op | approach | M1 Max × | Skylake × | Raptor Lake × |
|---|---|---|---|---|
| fmin | original: DD comparison + select | **5.5×** | **9.8×** | **9.5×** |
| fmax | original: DD comparison + select | **5.3×** | **13×** | **9.6×** |
| fdim | original: DD comparison, then subtract or zero | **6.1×** | **6.9×** | **8.4×** |
| copysign | original: sign-bit copy to hi, propagate to lo | 1.9× | **5.2×** | **6.9×** |
| fmod | sample: floor-multiple reduction loop; fallback to div chain | 0.85× | 0.76× | 0.82× |
| hypot | original: scaled sqrt(x²+y²) | **36×** | **20×** | **38×** |
| ldexp(.,5) | original: ldexp on both limbs | **2.0×** | **2.4×** | **4.0×** |

### Exponential / logarithmic

| op | approach | M1 Max × | Skylake × | Raptor Lake × |
|---|---|---|---|---|
| exp | Julia: exp2 polynomial (14-term Horner) + ldexp reconstruction | **3.1×** | **2.7×** | **3.8×** |
| exp2 | Julia: exp2 polynomial (14-term Horner) | **3.3×** | **3.1×** | **4.5×** |
| expm1 | original: exp(x) − 1 via DD sub | **4.2×** | **3.3×** | **3.9×** |
| log | Julia: log2 table lookup (32 centers) + polynomial (7-term Horner) | **4.4×** | **4.0×** | **5.1×** |
| log10 | Julia: log2 kernel × DD log10(2) | **5.9×** | **5.1×** | **6.4×** |
| log2 | Julia: log2 table lookup + polynomial | **5.6×** | **4.7×** | **6.3×** |
| log1p | original: log(1 + x) via DD add | **4.9×** | **4.3×** | **5.5×** |
| pow | Julia: exp(y × log(x)) | **4.3×** | **4.1×** | **5.3×** |
...
| cos | original: 13-term Taylor Horner + 3-part Cody–Waite π/2 + π/8 split | **2.9×** | **2.3×** | **2.9×** |
| tan | original: sin/cos Taylor kernels + DD divide | 1.3× | 1.1× | 1.3× |
| asin | original: Newton step on sin, seeded by libm asin(hi) | 1.9× | 1.7× | **2.1×** |
| acos | original: Newton step on cos, seeded by libm acos(hi) | 1.9× | 1.7× | **2.4×** |
...
| cosh | original: (exp+exp⁻¹)/2 | 1.6× | 1.5× | 2.0× |
| tanh | original: sinh/cosh (\|x\|<0.5) or (1−e⁻²ˣ)/(1+e⁻²ˣ) | **2.4×** | **2.2×** | **2.6×** |
| asinh | original: Taylor series (\|x\|<0.01) or log(x+√(x²+1)) with Newton | **6.2×** | **6.0×** | **7.7×** |
| acosh | original: log(x+√(x²−1)) with Newton correction | **5.7×** | **5.5×** | **8.2×** |
| atanh | original: Taylor series (\|x\|<0.01) or ½·log((1+x)/(1−x)) | **4.6×** | **4.4×** | **6.1×** |
...
| erfc | original: 1−erf(x), or libm erfc(hi) for large x | **5.1×** | **5.5×** | **7.5×** |
| tgamma | original: libm tgamma(hi), no lo correction | **103×** | **59×** | **81×** |
| lgamma | original: libm lgamma(hi), no lo correction | **72×** | **46×** | **55×** |
